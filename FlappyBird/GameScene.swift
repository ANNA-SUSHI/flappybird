...
//  FlappyBird
//
//  Created by Anna T on 2021/05/22.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate /* 追加 */ {
    ...
    let sound = SKAction.playSoundFileNamed("curiousing.mp3", waitForCompletion:false)
    ...
    func setupItem() {
        // アイテムの画像を読み込む
                ...
                // アイテム作成までの待ち時間(1秒)のアクションを作成
                let waitAnimation = SKAction.wait(forDuration: 1)
                // 待ち時間(1秒)->アイテムを作成->待ち時間(1秒)->待ち時間(1秒)を無限に繰り替えるアクションを作成
                // 壁は、壁を作成->待ち時間(2秒)->壁を作成を繰り返すのに対し、アイテムは1秒ずらして繰り返すことで壁と壁の間にアイテムを表示させる
